...
Ensure the response is **valid JSON** and contains **only the JSON object**, without additional text.
"""
...
def generate_image_metadata(image_path):
    ...
    try:
        ...
        # Run test only if the script is executed directly
if __name__ == "__main__":
    test_image = "test-image.jpg"  # Replace with a real image filename
    if os.path.exists(test_image):
        print(f"\n🚀 Running Gemini AI Test on: {test_image}")
        metadata = generate_image_metadata(test_image)
        ...
        if metadata:
            ...
            print("\n❌ Test Failed: No metadata generated.")
    else:
        print(f"\n❌ ERROR: Test image '{test_image}' not found.")
        print("📌 Place an image in this directory and rename it to 'test-image.jpg'.")
